...
async function startPlayback() {
  ...
  while (i < notes.length) {
    const note = notes[i]
    const playUnits = note.tupletGroup ? note.units * 2 / 3 : note.units; let duration = noteSeconds(playUnits)
    let j = i
    while (j < notes.length - 1 && notes[j].tiedToNext) { j++; const pu = notes[j].tupletGroup ? notes[j].units * 2 / 3 : notes[j].units; duration += noteSeconds(pu) }
...
async function exportAudioFile() {
  ...
  const rendered = await Tone.Offline(async () => {
    ...
    while (ei < exportNotes.length) {
      ...
      const epu = exportNote.tupletGroup ? exportNote.units * 2 / 3 : exportNote.units; let exportLength = noteSeconds(epu)
      let ej = ei
      while (ej < exportNotes.length - 1 && exportNotes[ej].tiedToNext) { ej++; const epu2 = exportNotes[ej].tupletGroup ? exportNotes[ej].units * 2 / 3 : exportNotes[ej].units; exportLength += noteSeconds(epu2) }
