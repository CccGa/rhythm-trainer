...
function makePatternCatalog() {
  // Base note lengths depend on meter:
  // /4: base=quarter, long=half
  // /8: base=eighth, long=quarter
  ...
  return [
    ...
    // Card "四分/八分": /4 -> quarter only; /8 -> eighth only
    { id: 'quarterEighth', notes: [baseUnits] },
    // Card "二分/四分": /4 -> half only; /8 -> quarter only
    ...
    // 大附点：前大附点后八 / 前八后大附点；其中 8 分可随机拆成两个 16 分
    { id: 'bigDot', notes: [6, 2] },
    { id: 'bigDot', notes: [2, 6] },
    { id: 'bigDot', notes: [6, 1, 1] },
    { id: 'bigDot', notes: [1, 1, 6] },
    // 大切分：两端 8 分可随机拆成两个 16 分
...
function fillBar(targetUnits, forceNoRest = false) {
  ...
  while (sumUnits(notes) < targetUnits) {
    ...
    if (choice.triplet) {
      ...
      const beamUnits = choice.occupy / 3
      choice.notes.forEach((units, index) => {
        ...
        if (shouldRest) {
          notes.push(makeNote(units, a4Pitch, choice.id, { isRest: true, occupiesUnits: occupyHere, beamUnits, tupletGroup, tuplet: choice.tuplet }))
          return
        }
        const pitch = randomItem(selectedPitches.value)
        notes.push(makeNote(units, pitch, choice.id, { occupiesUnits: occupyHere, beamUnits, tupletGroup, tuplet: choice.tuplet }))
      })
    } else {
      choice.notes.forEach((units, index) => {
...
async function generateQuestion() {
  ...
  if (useAnacrusis.value) {
    // Pickup does NOT consume bar count. It is always exactly 1 beat.
  ...
  // If tie is enabled, generate at least one (within-bar or cross-bar), spread across score.
  if (enabledPatterns.tie) {
    ...
    // Collect all candidate pairs with per-bar tracking
    const candidates = []
    ...
    // Shuffle for even distribution; try each with variable chance
    const barTies = {}
    ...
    for (let ci = 0; ci < shuffled.length && placed < maxTies; ci += 1) {
      ...
      // Penalize consecutive ties: very low probability (8%) if target already tied
    ...
    // Guarantee: if still none placed
  ...
  // Enforce minimum duration for the last note: no less than half a beat.
  {
    const allBars = pickupBar.value ? [pickupBar.value, ...questionBars.value] : questionBars.value
    ...
    if (lastNote) {
      ...
      if (!isEighthMeter.value) {
        minUnits = 2  // 4/4: half of a quarter = eighth
      } else if (eighthBeatMode.value === 'dottedQuarter') {
        minUnits = 3  // 6/8 dottedQuarter: half of a dotted quarter = 3 sixteenths
      } else {
        minUnits = 1  // 6/8 eighth mode: half of an eighth = sixteenth
      }
...
function groupedBeamNotes(bar, vexNotes) {
  ...
  // Force-beam tuplets as a whole (prevents "2-beamed + 1 loose" and prevents beaming with neighbors)
  const skip = new Set()
  ...
  tupletGroups.forEach((indices) => {
    if (indices.length >= 3) {
      groups.push(indices.map((i) => vexNotes[i]))
  ...
  for (let index = 0; index < bar.length; index += 1) {
    ...
    // Special case: 小附点(附点八分+十六分)需要连梁，否则会出现符尾不连上的视觉问题。
    if (
...
function renderStaff() {
  ...
  measures.forEach((bar, index) => {
    ...
    // Render tuplets (e.g. triplets)
    const tupletGroups = new Map()
    bar.forEach((note, noteIndex) => {
      if (!note.tupletGroup) return
      if (!tupletGroups.has(note.tupletGroup)) tupletGroups.set(note.tupletGroup, { notes: [], tuplet: note.tuplet })
      tupletGroups.get(note.tupletGroup).notes.push(vexNotes[noteIndex])
    })
    tupletGroups.forEach((group) => {
      if (group.notes.length >= 3) {
        new Tuplet(group.notes, { numNotes: group.tuplet?.numNotes || 3, notesOccupied: group.tuplet?.notesOccupied || 2 })
          .setContext(context)
          .draw()
      }
    })

    // Collect for cross-measure ties; draw ties after layout.
  ...
  // Draw ties after all notes have x-positions (supports within-bar and cross-bar).
  for (let i = 0; i < globalNotes.length - 1; i += 1) {
   ...
   if (!globalVex[i] || !globalVex[i + 1]) continue
    // Same row: normal tie; different rows: two partial ties
    if (cur._renderRow === next._renderRow) {
      ...
      // Second note: tie coming from left
...
function initAudio() {
  ...
  // Fallback is used only if sampler hasn't loaded yet. Keep it mellow (avoid "stringy" feel).
...
async function startPlayback() {
  ...
  stopPlayback()
  await Tone.start()
  // Ensure sampler buffers are ready so we don't fall back to the synth.
  await Tone.loaded()
  ...
  // Handle tied notes: combine durations, skip re-trigger for tied-to notes.
  {
    let i = 0
    ...
    while (i < notes.length) {
      ...
      let j = i
      while (j < notes.length - 1 && notes[j].tiedToNext) {
        j++
        duration += noteSeconds(notes[j].units)
      }
...
async function exportAudioFile() {
  ...
  const seconds = 3
    + referenceSeconds
    + fullBarSeconds() * 2
    + flatNotes.value.reduce((sum, note) => sum + noteSeconds(note.units), 0)
    + 1

    const rendered = await Tone.Offline(async () => {
      const exportSynth = new Tone.Synth({
        oscillator: { type: 'triangle' },
        envelope: { attack: 0.01, decay: 0.2, sustain: 0.25, release: 0.25 },
      }).toDestination()
    ...
    await Tone.loaded()
    let cursor = 0
    exportSynth.triggerAttackRelease('A4', 3, cursor)
    cursor += 3
    if (selectedPitches.value.length > 1) {
      sortedPitches.value.forEach((pitch) => {
    exportSynth.triggerAttackRelease(pitch.tone, 1.35, cursor)
        cursor += 1.5
      })
      cursor += 1
    }
    ...
    // Handle tied notes in export: combine durations, skip re-trigger for tied-to notes.
    {
      let ei = 0
      ...
      while (ei < exportNotes.length) {
        ...
        let ej = ei
        while (ej < exportNotes.length - 1 && exportNotes[ej].tiedToNext) {
          ej++
          exportLength += noteSeconds(exportNotes[ej].units)
        }
...
watch([timeSignature, bars, pitchCount, useAnacrusis, allowRests, eighthBeatMode, bpm], () => {
  generateQuestion()
})
watch(showScore, async () => {
  await nextTick()
  renderStaff()
})
